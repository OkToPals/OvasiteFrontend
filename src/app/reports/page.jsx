...
import { base_url, get_all_organizations_url } from "@/api_utils";
...
const Reports = () => {
  ...
  const get_all_reports = async(jwt) => {
    try {
        const response = await axios_instance.get(base_url + '/reports', {
          headers: {
  ...
  return (
    <main className="flex flex-col md:flex-row">
      <SidebarNav activeLink={"reports"} pagetitle={'Reports'} />
      ...
      <section className=" bg-mobile-bg md:ml-[25vw]  md:w-[75vw]">
        ...
        <header className="h-[6rem] hidden flex-row items-center justify-between py-[1.6rem] border-b border-ova_grey_border bg-ova_white md:fixed md:flex md:w-[75vw]">
          <h1 className="text-[1em] md:text-[1.5em] font-bold pl-[1.2rem]">Reports</h1>
         ...
         <div className="max-w-full flex flex-row justify-between md:justify-end items-center px-[1.2rem] pb-4 md:py-[1.5rem] mt-[6.5rem]">
            <h1 className="md:hidden text-[1em] md:text-[1.25em] font-extrabold ">Reports</h1>
      ...
      {/* {
        showEditModal ? <EditOrganizationModal 
